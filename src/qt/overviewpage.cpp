...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sPIVPercentage, QString& szbitPercentage)
{
    int nPrecision = 2;
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    if (fEnableZeromint) {
        ...
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in BitMoney.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in BitMoney.conf");
    }
    ...
    // zBIT
    bool showzbitAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzbitUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzbitImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzbitAvailable);
    ui->labelzBalanceMatureText->setVisible(showzbitAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzbitUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzbitUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzbitImmature);
    ui->labelzBalanceImmatureText->setVisible(showzbitImmature);

    // Percent split
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelPIVPercent->setVisible(showPercentages);
    ui->labelzbitPercent->setVisible(showPercentages);
