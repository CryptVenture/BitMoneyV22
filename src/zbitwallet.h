...
#ifndef BITMONEY_zbitWALLET_H
#define BITMONEY_zbitWALLET_H
...
public:
    CzbitWallet(std::string strWalletFile);
    ...
    void GenerateDeterministiczbit(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedTozbit(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BITMONEY_zbitWALLET_H
