...
CzbitWallet::CzbitWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zBIT seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzbitSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezbitSeed_deprecated()) {
                    LogPrintf("%s: Updated zBIT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zBIT seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zBIT wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzbitWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezbitCount(nCountLastUsed);
    else if (!walletdb.ReadzbitCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzbitWallet::Lock()
{
    seedMaster = 0;
}

void CzbitWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzbitWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozbit(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzbitWallet::LoadMintPoolFromDB()
{
...
void CzbitWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzbitWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzbitWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbitTracker->HasPubcoinHash(pMint.first)) {
...
bool CzbitWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozbit(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbitTracker which also adds to database
    pwalletMain->zbitTracker->Add(dMint, true);
...
void CzbitWallet::SeedTozbit(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzbitWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzbitWallet::GenerateDeterministiczbit(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzbitWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzbitWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
