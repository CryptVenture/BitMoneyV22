...
CzbitTracker::CzbitTracker(std::string strWalletFile)
{
...
CzbitTracker::~CzbitTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzbitTracker::Init()
{
...
bool CzbitTracker::Archive(CMintMeta& meta)
{
...
bool CzbitTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzbitTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzbitTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzbitTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzbitTracker::GetSerialHashes()
{
...
CAmount CzbitTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzbitTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzbitTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzbitTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzbitTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzbitTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzbitTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzbitTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzbitTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzbitTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzbitTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzbitTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzbitTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzbitTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzbitTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzbitTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzbitTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzbit from DB\n", __func__, listDeterministicDB.size());
    }
