...
public:
    ...
    bool WritezbitSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadzbitSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadzbitSeed_deprecated(uint256& seed);
    bool ErasezbitSeed();
    bool ErasezbitSeed_deprecated();

    bool WritezbitCount(const uint32_t& nCount);
    bool ReadzbitCount(uint32_t& nCount);
